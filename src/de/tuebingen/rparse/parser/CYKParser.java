...
/**
 * The CYK parser from Seki et al. (1991) to be used as Weighted Deductive
 * Parser (Nederhof, 2003).
 * 
 * @author wmaier
 */
public class CYKParser implements RCGParser {
	 ...
	 *             If any essential part of the parser data is not provided,
	 *             i.e., is null.
	 */
	...
	public boolean parseWithTimeout(ParserInput pi, int timeout)
		...
		try {
			if (deterministicBinarization && pd.doFilter)
				doParseDeterministicFilters(words, pi.getTags(),
						pi.tagsAsSet(), timeout);
			else
	 ...
	 * 
	 * @param words
	 *            The input words as integers, backed by the numberer in the
	 *            ParserData.
	 * @param tags
	 *            The input tags as integers, backed by the numberer in the
	 *            ParserData.
	 * @param tagset
	...
	public boolean doParseDeterministicFilters(int[] words, int[] tags,
			Set<Integer> tagset, int timeout) throws TimeoutException {
		...
		CYKItem nit = null;
		// Holds all items which can be created from another one. After all of
		// them
